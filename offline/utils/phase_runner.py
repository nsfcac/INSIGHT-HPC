...
# Run a stage function, catching and reporting any exception.
...
# Resolve which stages to run from the --only / --from options.
def select_stages(
    order: list, only: Optional[str] = None, from_stage: Optional[str] = None
) -> list:
    if only:
        return [only]
    if from_stage:
...
# Register the shared --force/--from/--only CLI arguments.
...
# Run the selected stages in order, timing each and collecting pass/fail results.
def run_stages(
    title: str,
    order: list,
    labels: dict,
    ...
    *,
    ...
    for key in stages:
        ...
        if capture_errors:
            ...
            runners[key](force)
            ok = True
        results[key] = ("ok" if ok else "FAILED", round(time.perf_counter() - ts, 1))
