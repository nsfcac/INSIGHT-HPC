...
from src.phase3.job_context_annotator_module.annotation import *
...
def run_job_context_annotator(force: bool = False) -> None:
    ...
    feat_base = Path(cfg["paths"].get("features_aligned", "data/features_aligned"))
    if not feat_base.exists():
        feat_base = Path(cfg["paths"]["features"])

    master_base = Path(cfg["paths"].get("master", "data/master"))
