...
def main() -> None:
    ...
    gt_dir = Path(
        cfg.get("ground_truth_dir", p4_cfg.get("ground_truth_dir", "data/ground_truth"))
    )
    ...
    if not any((gt_dir / f).exists() for f in tpl_files):
        from src.phase4.ground_truth_loader import write_templates
    ...
    def step1(f):
        from src.phase4.ground_truth_loader import run_load_ground_truth
    ...
    def step2(f):
        from src.phase4.synthetic_ground_truth import generate
    ...
    def step3(f):
        from src.phase4.score_fusion import run as run_score_fusion

        run_score_fusion(force_retrain=f)
        import json

        rep_dir = Path(cfg["phase4"].get("reports_dir", "reports/phase4_eval"))
    ...
    out_dir = p4_cfg.get("output_dir", "data/phase4")
    rep_dir = p4_cfg.get("reports_dir", "reports/phase4_eval")
    print(f"""
  Key outputs:
