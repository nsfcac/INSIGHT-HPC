...
from src.utils.io_utils import load_config, load_public_tables, load_metric_parquet, save_parquet, build_lookup_dicts

# Files with at least this many RAW rows are split into per-node-group chunks
...
# Flag rolling z-score spikes per host.
def flag_spikes(df: pd.DataFrame, col: str, z_thresh: float, rolling_pts: int, min_abs_delta: float = 0.0) -> np.ndarray:
    ...
    for _, idx in df.groupby("hostname", sort=False).indices.items():
        ...
        abs_flag = (abs_dev > min_abs_delta) if min_abs_delta > 0.0 else np.ones(n, dtype=bool)
        out[idx] = (z_flag & abs_flag).astype(np.int8)

    return out


# Flag runs of unchanging values (flatlines) per host.
...
# Resolve a per-metric override with short-name fallback.
...
# Compute the flatline min-run length (points) for a metric.
def flatline_window(metric: str, audit_cfg: dict, interval: int) -> int:
    overrides = audit_cfg.get("flatline_overrides", {})
    minutes   = resolve_metric_override(metric, overrides, audit_cfg["flatline_window_min"])
    if minutes == -1:
        return -1
    return max(3, int(minutes * 60 / interval))


# Compute per-sensor QC flags (dup/spike/flatline/bad-range) for one metric.
def audit_df(df: pd.DataFrame, metric: str, source_name: str, cfg: dict, thresholds: pd.DataFrame = None, comp: str = None, verbose: bool = True) -> pd.DataFrame:
    ...
    if source_cfg.get("skip_audit", False):
        ...
        if verbose:
            print(f"      audit: skipped (skip_audit=true)  "
    ...
    comp_ranges    = source_cfg.get("per_component_ranges", {}).get(comp, {}) if comp else {}
    ranges         = comp_ranges.get(metric) or default_ranges.get(metric)

    spike_overrides    = audit_cfg.get("spike_zscore_overrides", {})
    effective_z        = resolve_metric_override(metric, spike_overrides, audit_cfg["spike_zscore"])
    ...
    if enable_dup and "_dup_count" in df.columns:
        ...
        dup_row = (df["_dup_count"].to_numpy() > max(multiplier * median_count, 2)
    ...
    for col in avg_cols:
        ...
        if enable_spike and effective_z != -1:
            ...
            col_flags[col] |= flag_spikes(
                df, col, float(effective_z), rolling_pts,
...
# Summarize flag counts and percentages for an audited frame.
...
# Write the audit summary JSON to the reports dir.
...
# Load the dynamic metric-thresholds CSV keyed for lookup.
def load_thresholds(cfg: dict):
    ...
    df["lookup_key"] = (
        ...
        df["source_type"] + "_" +
        df["metric"] + "_" +
        df["fqdd"].fillna("none") + "_" +
        df["source"].fillna("none")
    )
    return df.set_index("lookup_key")


# Decode threshold rows into per-sensor keys via fqdd/source maps.
def make_sensor_thresholds(thresholds: pd.DataFrame, fqdd_map: dict, source_map: dict) -> pd.DataFrame:
...
# Resolve audit worker count from env or SLURM CPUs (cap 6).
...
# Number of per-node-group chunks to split each giant idrac file into (1 = no split).
def audit_split() -> int:
    try:
...
# Resolve the heartbeat log interval (seconds) from env.
...
# Return a parquet's row count, or 0 on failure.
...
# Audit one node-group of a metric file and write a partial parquet.
def audit_chunk(parquet_path: Path, comp: str, source: str, source_dir: Path, partial_out: Path, cfg: dict, pub_tables: dict, comp_thresholds, node_ids):
    metric = parquet_path.stem.lower()
    buf = io.StringIO()
    with redirect_stdout(buf):
        df = load_metric_parquet(comp, source_dir, parquet_path, pub_tables, cfg,
                                 verbose=False, node_ids=node_ids)
        if df is None or df.empty:
            return partial_out, 0, buf.getvalue()
        df = audit_df(df, metric, source, cfg, thresholds=comp_thresholds,
                      comp=comp, verbose=False)
...
# Audit one metric file. Top-level/picklable so a process pool can run it.
def audit_one_file(parquet_path: Path, comp: str, source: str, source_dir: Path, out_path: Path, force: bool, cfg: dict, pub_tables: dict, comp_thresholds, verbose: bool):
    metric = parquet_path.stem.lower()
    ...
    summary = (f"    DONE {metric:30s}  {n_rows:>12,} rows × {n_cols} cols  "
...
# Audit every raw metric parquet in parallel and write flagged outputs.
def audit_raw_files(force: bool = False, verbose: bool = True) -> None:
    ...
    for comp_cfg in cfg["components"]:
        ...
        for source in ["idrac", "slurm", "pdu"]:
            ...
            split   = audit_split()
            node_ids_all = (sorted(build_lookup_dicts(pub_tables)[0].keys())
                            if split > 1 else [])
            if split > 1 and source == "idrac":
                print(f"  [{source.upper()}] workers={workers} split={split} "
                      f"node_groups={min(split, len(node_ids_all))}", flush=True)

            whole_tasks, chunk_tasks, merge_specs = [], [], []
            for p in parquets:
                metric   = p.stem.lower()
                out_path = out_base / comp / source / f"{metric}.parquet"
                do_split = (split > 1 and source == "idrac" and (force or not out_path.exists())
                ...
                if do_split:
                    ...
                    for gi in range(split):
                        ...
                        partials.append(pp)
                        chunk_tasks.append((p, comp, source, source_dir, pp, cfg,
                                            pub_tables, comp_thresholds, set(g)))
                    merge_specs.append((metric, out_path, partials))
                else:
                    whole_tasks.append((p, comp, source, source_dir, out_path,
                                        force, cfg, pub_tables, comp_thresholds, verbose))
            ...
            if workers == 1 or n_tasks <= 1:
                for t in whole_tasks:
                    r = audit_one_file(*t); print(r[3], flush=True); results.append(r)
                    ...
                    while pending:
                        if heartbeat > 0:
                            ...
                            if not done:
                                ...
                                print(f"    ... audit running  done={done_count}/{n_tasks}  "
            ...
            for metric, final_out, partials in merge_specs:
                frames = [pd.read_parquet(pp, engine="pyarrow")
                          for pp in partials if pp.exists()]
                ...
                if stats:
                    ...
                    total_files += 1
                n_rows = len(full)
                del full, frames; gc.collect()
                for pp in partials:
                    try:
                        pp.unlink()
                    except OSError:
                        pass
                print(f"    DONE {metric:30s}  {n_rows:>12,} rows  "
                      f"(merged {len(partials)} chunks)", flush=True)
            gc.collect()
    ...
    print("\nAudit complete")
    print(f" - files={total_files}  rows={total_rows:,}  "
